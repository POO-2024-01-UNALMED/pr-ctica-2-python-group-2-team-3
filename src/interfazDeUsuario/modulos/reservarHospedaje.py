...
from modulos.reservarActividades import realizarReserva,comprarSuscripcion
...
def reservarHospedaje(ventana_usuario, opcion=0, seleccion=None):
        ...
        if opcion==2:
                if seleccion:
                        #ventana_usuario.reserva=buscarReserva(seleccion)
                        pass
                ventana_usuario.borrarFrame(ventana_usuario.resultados_frame)
                ventana_usuario.tituloResultados()
                ventana_usuario.frameResultados(["Reserva:"],[""])#ventana_usuario.reserva.toString()
...
def reservarHotel(ventana_usuario,opcion=0,seleccion=None,textoBase=None):
        ...
        hoteles_disponibles=Hotel.hotelesDisponibles(ventana_usuario.reserserva)
        if opcion==0:
        ...
        if opcion==2:
                if seleccion == "Cambiar":
                        ...
                        ventana_usuario.modificarTexto("".join(textoBase)+"Procede a ingresar el numero de habitaciones que quieres reservar por cada tipo, ten en cuenta que cada tipo de habitacion\n"
                        ...
                        ventana_usuario.crearFormulario(tipo_formulario=3, on_accept=lambda seleccion: reservarHotel(ventana_usuario, 3, seleccion), criterios=criterios, verificaciones=excepcionesHotel2)
